...
<?php
	...
	{	
		global $htpath;
		if($_POST["reportname"] and $_POST["password"])
		{
		$curdir = realpath($htpath."reports");
		exec ("del /Q ".$curdir);
//		if(is_file($curdir."\\".$_POST["reportname"].".exe"))
//		{
//			//echo "del /Q ".$curdir."\\".$_POST["reportname"].".exe";
//			//exec("del /Q ".$curdir."\\".$_POST["reportname"].".exe");
//		}
		$cmd = realpath($htpath."\\7z\\7z.exe")." a -sfx7z.sfx -p".$_POST["password"]." ".$curdir."\\".$_POST["reportname"].".exe ".$curdir."\\reports\\"; 		
	
		exec($cmd);
		exec("rmdir /S /Q ".realpath($curdir."\\reports\\"));
		echo "Reports are finished.";
		}
		else
		{
			echo "Report name and password not passed in";
		}
	}
	...
	function saveFile($user)
	{
		...
		exec("mkdir ".realpath($htpath."reports")."\\reports");
		$wrt->save(realpath($htpath."reports\\reports")."\\".$user.".xlsx");
	...
	function main()
	{
		global $objPHPExcel,$wksVid,$wksDoc,$wksQuiz,$wksChro, $settingsread,$contrackingdsn,$contrackingusr,$contrackingpwd,$sqlallreport;
		
		$P = new PDO($contrackingdsn, $contrackingusr, $contrackingpwd);
	
		$PS = $P->query($sqlallreport);
		...
		if($settingsread['reports']['encrypt_reports'] == 1)
		{
			archiveReports();
		}
	}
